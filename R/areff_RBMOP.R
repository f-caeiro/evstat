#############################################################
###   Computation of argmax(p) {AREFF}
#############################################################
optimalp <- function(rho, method="lehmer"){
  ...
  # aproximation
  if (method=="lehmeraprox") pp <- .1487*rho^3+.6537*rho^2+1.0621*rho+1.949
  # implicit equation
  if (method=="lehmer"){
    for (i in 1:length(rho)){pp[i] <- uniroot(function(x, r){digamma(2*x-1)-digamma(x)+log(1-r)/(2*r)}, interval=c(1, 2), tol = 1e-10,r=rho[i])$root}     
  }
  if (method=="mop"){
    pp <- (1-rho/2)-sqrt((1-rho/2)^2-1/2)  
  }  
  return(pp)
}
#rr <- -rev(1:80/40)
#out <- optimalp(rr, "lehmernm")
#cbind(rr,out)
#out2<-optimalp(rr, "lehmeraprox")
#plot(out-out2)
#r0 <- c(-3,-2.5,-2,-1.5,-1,-0.75,-0.5,-0.25,-0.1,-0.01)
#optimalp(r0, "lehmer")
#
#
bias <- function(type, xi, rho, zeta=1, r=1){
  ...
  if (type=="whill-le") b <- (1+r)/(1-rho)-(2*r)/(2-rho)
...
sigma2 <- function(type, xi, rho, r=1){
  if(is.element(type, c("Hill", "CH", "NCH"))) {s2 <- xi^2}
  ...
  if (type=="whill-le") s2 <- xi^2*(1+r^2/3)
  ...
  if(type=="Lehmer0"){
    p0 <- optimalp(rho, "lehmer")
    s2 <- xi^2*gamma(2*p0-1)/(gamma(p0)^2)
    }
  return(s2)
}
...
# AREFF (RBKernelP|CH)
pdf(file = "fig_areff_rbkp-ch.pdf", width=9, height=6)
par(mar=c(5,4,4,2))
...
contour(x2, y2, z2, add=TRUE, levels=c(0.6, 0.7, 0.8, 0.85, 0.9, 0.95, 0.98, 1.02, 1.05, 1.1, 1.2, 1.3,1.5,1.75, 2, 3, 4, 5), col="black", labcex=0.9, lwd=1, method="flattest")  #
...
# 1/areff(x=1, y=-1, est1="KernelPRB", est2="CH", p1="r", p2="rho", zeta=1, RB=TRUE)
# 1/areff(x=2, y=-2, est1="KernelPRB", est2="CH", p1="r", p2="rho", zeta=1, RB=TRUE)
# areff(x=1, y=-1, est1="KernelP", est2="CH", p1="r", p2="rho", zeta=1, RB=TRUE)
# areff(x=1156, y=-0.5, est1="KernelP", est2="CH", p1="r", p2="rho", zeta=1, RB=TRUE)
...
lmse<-function(w, rho=-1, zeta=1, type="RBKernelP"){
  ...
  #b2<-bias(type="CH", xi=1, rho=rho, zeta=zeta, r=0)
  #vv <- ((2*w+1)/(w+1)^2)^(-2*rho)
  #b1 <- (1+w)*(zeta/(1+w-2*rho)-(1+w)/(1+w-rho)^2)
  #b2 <- zeta/(1-2*rho)-1/(1-rho)^2
  lmse0 <- (-4*rho)*log(v1)+2*log(abs(b1))
  return(lmse0)
}
optim(parm <- 1, fn=lmse, rho=-1.75, method="BFGS", hessian=F, control=list(maxit=5000))


lmse(w=1,rho=-1,zeta=1)  

1/lmse(w=1,rho=-1,zeta=1) 



wopt<-function(rho=-1, zeta=1){
  w <- optim(parm <- 2, fn=lmse, rho=rho, zeta=zeta, method="BFGS", hessian=T, control=list(maxit=2000))
  return(w$par)
}
#wopt(rho=-2,zeta=-1)
wopt(rho=-1,zeta=5/3)
...
qm <- matrix(data = NA, nrow = n1, ncol = n2)
for (i in 1:n1){
  for (j in 1:n2){ qm[i,j]<-wopt(rho=yr[i],zeta=xz[j])}
} 
...
image(x, y, z, col=gray(1), axes=TRUE, zlim=c(0.95,0.999), xlab=expression(xi), xlim=c(0,1.5), ylim=c(-2,0), las=1, ylab=expression(rho), cex.axis=0.9, cex.lab=1.0, main=expression(AREFF[list(MOM,GH)/Hill]))
...
y <- -(800:1)/400
z <- outer(x, y, "areff", est1="MOM", est2="Hill", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.25, 0.5,.75,2), col="black", labcex=0.75, lwd=1)  #
contour(x, y, z, add=TRUE, levels=c(1), col="blue", labcex=0.75, lwd=1)  #
x <- (1:60)/400
y <- -(800:1)/400
z <- outer(x, y, "areff", est1="MOM", est2="Hill", p1="xi", p2="rho", RB=F)
...
y <- -(800:1)/400
z <- outer(x, y, "areff", est1="MOM", est2="Hill", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.95,.97,.99,1.025,1.05), col="black", labcex=0.75, lwd=1)  #
x <- (1:200)/400
y <- -(35:1)/400
z <- outer(x, y, "areff", est1="MOM", est2="Hill", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1.025,1.05), col="black", labcex=0.75, lwd=1, drawlabels = F)  #
x <- (150:251)/400
y <- -(150:5)/400
z <- outer(x, y, "areff", est1="MOM", est2="Hill", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1.025,1.05), col="black", labcex=0.75, lwd=1, drawlabels = F)  #
...
z <-matrix(-1, nrow = length(x), ncol = length(y))
text(x=.27, y = -1/3, labels=expression(infinity), col="red3", cex=1.0)
...
#R2
# MOM/Lehmer0
filename<-paste("areff_mom-lp",".pdf",sep="")
pdf(file = filename, width=8, height=6)
par(mar=c(5,4,4,2))
#x <- (1:450)/300
x <- (1:150)/150
y <- -(200:1)/100
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
#zz<-sort(z, decreasing=T)
#zz<-round(log(zz[zz>1]),1)
image(x, y, z, col=gray(1), axes=TRUE, zlim=c(0.95,0.999), xlab=expression(xi), xlim=c(0,1.4), ylim=c(-2,0), las=1, ylab=expression(rho), cex.axis=0.9, cex.lab=1.0, main=expression(paste(AREFF[list(MOM,GH)/L],"*")))
image(x, y, z, col=gray(0.85), axes=TRUE, zlim=c(1.001,10000), add=TRUE)
...
y <- -(600:11)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.2, 0.3, 0.5,.75,2), col="black", labcex=0.75, lwd=1)  #
contour(x, y, z, add=TRUE, levels=c(1), col="blue", labcex=0.75, lwd=1)  #
x <- (1:60)/400
y <- -(600:1)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.2, 0.3, 0.5,.75,2), col="black", labcex=0.75, lwd=1,drawlabels = F)  #
x <- (1:30)/300
y <- -(150:1)/200
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.1), col="black", labcex=0.75, lwd=1,drawlabels = F)
...
y <- -(399:150)/200
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.1), col="black", labcex=0.75, lwd=1)
x <- (351:400)/400
y <- -(11:1)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1), col="blue", labcex=0.75, lwd=1,drawlabels = F)  #
...
y <- -(600:1)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(0.93,0.94, 0.95,0.96,.97,0.98,.99,0.998,1.02,1.05), col="black", labcex=0.75, lwd=1)  #
x <- (1:30)/300
y <- -(15:1)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1.02,1.05), col="black", labcex=0.75, lwd=1, drawlabels = F)  #
x <- (30:120)/300
y <- -(60:1)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1.02,1.05), col="black", labcex=0.75, lwd=1, drawlabels = F)  #
...
y <- -(300:225)/300
z <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(1.05), col="black", labcex=0.75, lwd=1, drawlabels = F)  #
#
curve(x/(x-1), from = 0, to = 0.23, add = TRUE, col = "red3")
curve(x/(x-1), from = 0.28, to = 0.7, add = TRUE, col = "red3")
text(x=.25, y = -1/3, labels=expression(infinity), col="red3", cex=1.0)
...
# MM/lehmer*
filename<-paste("areff_mm-lp",".pdf",sep="")
pdf(file = filename, width=8, height=6)
par(mar=c(4.5,4,4,2))
x <- (1:280)/200
y <- -(300:1)/150
z <- outer(x, y, "areff", est1="MM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
#zz<-sort(z, decreasing=T)
#zz<-round(log(zz[zz>1]),1)
image(x, y, z, col=gray(1), axes=TRUE, zlim=c(0.95,0.999), xlab=expression(xi), xlim=c(0,1.4), ylim=c(-2,0), las=1, ylab=expression(rho), cex.axis=0.9, cex.lab=1.0, main=expression(paste(AREFF[list(MM)/L],"*")))
image(x, y, z, col=gray(0.85), axes=TRUE, zlim=c(1.001,10000), add=TRUE)
abline(h=seq(-2.0,-0.25, by=0.25), v=0, lty=2, col=gray(0.4))
abline(v=seq(0,2, by=0.1), lty=2, col=gray(0.4))
#
...
y <- -(145:1)/300
z <- outer(x, y, "areff", est1="MM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
contour(x, y, z, add=TRUE, levels=c(.985,.99, .995), col="black", labcex=0.8, lwd=1,drawlabels = T)  #
...
# M/GH/MM/Hill
#deepskyblue2
filename<-paste("areff_m_gh_mm_lp",".pdf",sep="")
pdf(file = filename, width=8, height=7)
#c(bottom, left, top, right)
par(mar=c(4.5,4,1,2))
x <- (1:420)/300
y <- -(400:1)/200
z1 <- outer(x, y, "areff", est1="MOM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
z2 <- outer(x, y, "areff", est1="MM", est2="Lehmer0", p1="xi", p2="rho", RB=F)
#z1[z1<1]<-0
z1<-ifelse(z1<1,0,z1)
z2<-ifelse(z2<1,0,z2)
z1<-ifelse(z1>z2,z1,0)
z2<-ifelse(z2>z1,z2,0)
image(x, y, z1, col="lightsteelblue1", axes=TRUE, zlim=c(.99,10^3), xlab=expression(xi), xlim=c(0,1.4), ylim=c(-2,0), las=1, ylab=expression(rho), cex.axis=0.9, cex.lab=1.0, main="")
image(x, y, z2, col="steelblue3", axes=TRUE, zlim=c(.99,10^3), add=TRUE)
abline(h=seq(-1.75,-0.25, by=0.25), v=0, lty=2, col=gray(0.4))
abline(v=seq(0.1,1.4, by=0.1), lty=2, col=gray(0.4))
text(0.25,-0.9,"L*", cex=1.4)
text(1.05,-1.65,"L*", cex=1.4)
text(1.05,-0.67,"MM", cex=1.4)
text(1.25,-0.17,"L*", cex=1.4)
text(0.61,-1.18,"M, GH", cex=1.4)
box()
dev.off()
...
#CMMSE2020
areff(x=-.1,y=-.5, est1="whill-le", est2="Hill", p1="r", p2="rho", zeta=1, RB=F)
areff(x=-5,y=-2, est1="whill-le", est2="Hill", p1="r", p2="rho", zeta=1, RB=F)
...
cl0<-colorRampPalette(c("#3B9AB2", "#78B7C5"), space = "Lab")(20)
lev0<-seq(from=0.375, to=1, length.out=21)
cl1<-colorRampPalette(c( "#EBCC2A", "#E1AF00", "#F21A00"), space = "Lab")(30)
lev1<-c(seq(from=1, to=10, length.out=29),1000)





filename<-paste("areff_wh_h",".pdf",sep="")
pdf(file = filename, width=10, height=7)
# parte 1 - zonas a cinzento
x <- (1:499)/100
y <- -(199:1)/100
z <- outer(x, y, "areff", est1="whill-le", est2="Hill", p1="r", p2="rho", zeta=1, RB=F)
image(x, y, z, col=gray(.9), axes=TRUE, zlim=c(1,1000), xlab=expression(a), xlim=c(0,5), ylim=c(-2,0), las=1, ylab=expression(rho), cex.axis=1, cex.lab=1.0, 
      main=expression(AREFF[paste(WH(a)," | ",H,sep="")]))

Zissou1 <-c("#3B9AB2", "#78B7C5", "#EBCC2A", "#E1AF00", "#F21A00")
cl0<-colorRampPalette(Zissou1, space = "Lab")(50)
z0<-sort(z[z>1])
lev0<-c(seq(from=0.375, to=1, length.out=20)[1:19],1,quantile(z0, prob=1:31/31))
#barplot(rep(1,50), col=rev(cl0))
#for (i in 1:50) image(x, y, z, col=cl0[i], axes=TRUE, zlim=lev0[i:(i+1)], add=TRUE)
abline(v=seq(from=0.5, to=4.5, by=0.5), lty=2, col=gray(0.4))
abline(h=seq(from=-1.5, to=-0.5, by=0.5), lty=2, col=gray(0.4))

box()
# parte 2 - curvas de nivel (necessita de uma matriz com mais linhas e colunas)
x2<-(0:499)/100
y2 <- -(400:1)/200
z2 <- outer(x2, y2, "areff", est1="whill-le", est2="Hill", p1="r", p2="rho", zeta=1, RB=F)
contour(x2, y2, z2, add=TRUE, levels=c(0.4,0.5, 0.6, 0.7, 0.8, 0.9, 1.01, 1.1, 1.4, 2), col="black", labcex=0.9, lwd=1, method="flattest")  #
contour(x2, y2, z2, add=TRUE, levels=1, col="blue", labcex=0.9, lwd=1, method="flattest")
contour(x2, y2, z2, add=TRUE, levels=c(1.05), col="black", labcex=0.9, lwd=1, method="flattest",drawlabels = F)  #

curve(-2/(x-1), from = 2, to = 3.9, add = TRUE, col = "red3")
curve(-2/(x-1), from = 4.1, to = 5, add = TRUE, col = "red3")
text(x=4,y=-2/3, label=expression(infinity), col="red3")
